...
const CONTENT_CLASS = "privacy-content mt-6 prose prose-slate max-w-none text-gray-700 [&_p]:mb-3 [&_ul]:my-3 [&_ol]:my-3 [&_li]:ml-4 [&_strong]:font-semibold [&_h1]:text-2xl [&_h1]:font-bold [&_h1]:mt-6 [&_h2]:text-xl [&_h2]:font-bold [&_h2]:mt-5 [&_h3]:text-lg [&_h3]:font-bold [&_h3]:mt-4 [&_a]:text-[#255cdc] [&_a]:underline [&_a]:hover:no-underline";
...
export default function TermsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        console.error("Error loading terms of service:", e);
        setData(null);
      } finally {
        setLoading(false);
      }
  ...
  const source = data?.termsSource || "text";
  const docUrl = data?.termsDocUrl || "";
  const docName = (data?.termsDocName || "").toLowerCase();
  const text = data?.termsText || "";
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 md:py-12">
      ...
      {source === "doc" && docUrl ? (
        isPdf ? (
          ...
          <div className="mt-6 p-6 bg-gray-50 rounded-xl border border-gray-200">
            <p className="text-gray-600 mb-4">
              Terms of service are available as a document. Download it to view.
            </p>
            <a
              href={docUrl}
              target="_blank"
        ...
        <p className="mt-6 text-gray-500">Terms of service have not been set yet. Please check back later.</p>
      )}
    </div>
  );
}
